...
async function migrateData() {
  ...
  // 1. Read JSON file (using the requested (1) version)
  const filePath = path.join(__dirname, '../simba_products (1).json');
  ...
  const { data: existingBranches } = await supabaseAdmin.from('branches').select('name');
